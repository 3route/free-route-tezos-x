...
// PRICE_XTZ / PAY_SYMBOL are the per-run knobs (the exact line scripts/setup.ts prints).
...
// fulfill_ask. Run:  ASK_ID=2 PRICE_XTZ=0.004 PAY_SYMBOL=USDC npm run example
...
// ── config from env (.env + CLI). ASK_ID / PRICE_XTZ / PAY_SYMBOL are the per-run knobs setup.ts prints. ──
...
const group = buildBatchTransaction(swapOps, objkt.buildFulfillAsk({ marketplace: OBJKT_MARKETPLACE, askId: ASK_ID, editions: 1, amountMutez: PRICE_MUTEZ }));
