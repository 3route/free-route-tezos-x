// scripts/shared/setup.ts — bootstrap a runnable demo from scratch:
...
// Then prints the ready buy commands for both sides (Michelson + EVM), with ASK_ID / PAY_SYMBOL.
...
// 3) FUND the pay-token for whoever will buy: the Michelson buyer's alias, and (if EVM_SK is set) the EVM
//    account too. Both are paid by the Michelson buyer (XTZ -> pay-token, receiver = the owner) — one
//    Michelson op-group each, so the EVM account spends no gas here (it still needs gas XTZ on its 0x).
const payToken = await findToken(freeRoute, PAY_SYMBOL);
const fmtPay = (x: bigint) => `${Number(x) / 10 ** payToken.decimals} ${PAY_SYMBOL}`; // base units -> human-readable
const balanceOf = (owner: string) => readErc20Balance({ evmRpc: EVM_RPC, token: payToken.address, owner });

// The ERC20 balance settles on the EVM side shortly after the Tezos confirmation — poll up to ~15s.
const pollBalance = async (owner: string, min: bigint, tries = 15): Promise<bigint> => {
  let bal = await balanceOf(owner);
  for (let i = 0; i < tries && bal < min; i++) {
    await new Promise((r) => setTimeout(r, 1000));
    bal = await balanceOf(owner);
  }
  return bal;
};

// what a buy (payToken -> XTZ, exact-out sized to cover the price) will spend — ~same for either owner (same route)
...
// top up `owner` to ≥ needed pay-token, paid by the Michelson buyer (exact-out XTZ -> pay-token, receiver = owner)
const topUp = async (label: string, owner: string): Promise<void> => {
  const have = await balanceOf(owner);
  console.log(`${label}: have ${fmtPay(have)} · need ${fmtPay(needed)}`);
  if (have >= needed) {
    console.log(`  already funded — skip`);
    return;
  }
  const target = needed * 2n; // 2x margin for price drift + the buy's fresh re-quote
  const fundSwap = await freeRoute.getSwap({ src: XTZ.address, dst: payToken.address, amount: target - have, isExactOut: true, from: aliasAddress, receiver: owner, slippageBps: 300 });
  const fundOps = freeRoute.michelson.buildSwapOperation({ swap: fundSwap, srcAddress: XTZ.address });
  console.log(`  fund ${fmtPay(target - have)} (router ${fundSwap.tx.to})`);
  console.log(`  ${TZKT_EXPLORER}/${await sendGroup(buyer, fundOps)}`);
  const now = await pollBalance(owner, needed);
  console.log(`  now = ${fmtPay(now)}`);
  if (now < needed) throw new Error(`${label} still short after funding (${fmtPay(now)} < ${fmtPay(needed)})`);
};

await topUp('alias', aliasAddress);

// if EVM_SK is set, also top up the EVM account's pay-token (EVM-signed, the account pays its own XTZ).
// Loaded lazily so a Michelson-only setup needs no EVM env.
const evmAddr = evmAddressFromEnv();
if (evmAddr) {
...
console.log(`\n✅ ready. Buy the ask:`);
console.log(`   Michelson (Temple):  ASK_ID=${askId} PAY_SYMBOL=${PAY_SYMBOL} npm run example-buy:michelson`);
if (evmAddr)
  console.log(`   EVM (MetaMask):      ASK_ID=${askId} PAY_SYMBOL=${PAY_SYMBOL} npm run example-buy:evm`);
else console.log(`   EVM (MetaMask):      set EVM_SK (with gas XTZ on its 0x) and re-run setup to auto-fund ${PAY_SYMBOL}, then npm run example-buy:evm`);
